...
class DipRuleTest extends BaseArchTest {
    ...
    static final ArchRule DIP_001_infrastructure의_외부_클라이언트는_service의_인터페이스를_구현해야_한다 =
            classes()
                    ...
                    .should(implementInterfaceInCorePackages())
    ...
    private static ArchCondition<JavaClass> implementInterfaceInCorePackages() {
        return new ArchCondition<>("도메인이나 서비스 패키지의 인터페이스를 구현해야 함") {
            @Override
            public void check(JavaClass item, ConditionEvents events) {
                boolean matches = item.getRawInterfaces().stream()
                        .anyMatch(it -> it.getPackageName().contains(".domain") ||
                                it.getPackageName().contains(".service"));

                if (!matches) {
                    String message = String.format("%s 클래스가 도메인/서비스 인터페이스를 구현하지 않았습니다.", item.getName());
