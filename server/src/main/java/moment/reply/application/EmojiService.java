...
public class EmojiService {
    ...
    public EmojiCreateResponse addEmoji(EmojiCreateRequest request, Authentication authentication) {
        ...
        rewardService.rewardForEmoji(comment.getCommenter(), Reason.POSITIVE_EMOJI_RECEIVED, comment.getId());
    ...
    private void cancelRewardIfLastEmoji(Comment comment) {
        if (!emojiRepository.existsByComment(comment)) {
            rewardService.rewardForEmoji(comment.getCommenter(), Reason.CANCEL_POSITIVE_EMOJI_RECEIVED, comment.getId());
        }
    }
}
